...
class PlayerService:
    def __init__(self, injector, players_endpoint, characters_endpoint):
        self.__name__ = "PlayerService"
        self.logger = LoggerFactory.get_logger(self.__name__)
        self.injector = injector
        self.players_endpoint = players_endpoint
        self.characters_endpoint = characters_endpoint
    ...
    def get_accounts(self):
        return requests.get(self.players_endpoint).json()

    def get_account_by_id(self, account_id):
        url = self.players_endpoint+"/"+account_id
        self.logger.debug("GET: " + url)
        return requests.get(url).json()

    def get_account_by_name(self, account_name):
        url = self.players_endpoint+"/name/"+account_name
        print(f"GET: {url}")
        self.logger.debug("GET: " + url)
        return requests.get(url).json()

    def get_characters(self):
        return requests.get(self.characters_endpoint).json()

    def get_player_characters(self, account_id):
        url = self.characters_endpoint + "/account/" + account_id
        self.logger.debug("GET: " + url)
        return requests.get(url).json()

    def get_character(self, character_id):
        parameters = {"characterId": character_id}
        self.logger.debug("GET: " + self.characters_endpoint + ", PARAMS=" + str(parameters))
        return requests.get(self.characters_endpoint, params=parameters).json()

    def create_character(self, sheet):
        self.logger.debug("POST: " + self.characters_endpoint + ", SHEET=" + sheet)
        r = requests.post(self.characters_endpoint, json=sheet)
    ...
    def delete_character(self, character_id):
        self.logger.debug("DELETE: " + self.characters_endpoint + ", CHARACTER_ID: " + character_id)
        return requests.delete(self.characters_endpoint, json={"characterId": character_id}).json()
    ...
    def create_account(self, account_name, password):
        ...
        r = requests.post(self.players_endpoint, data=data)
        if r.status_code == 201:
            return r.json()["id"]
        else:
    ...
    def update_account(self, account_id, account_name=None, password=None):
        ...
        r = requests.put(f"{self.players_endpoint}/{account_id}", data=data)
    ...
    def update_character(self, character_id, character_name=None):
        ...
        r = requests.put(f"{self.characters_endpoint}/{character_id}", data=data)
        return r.status_code == 204
