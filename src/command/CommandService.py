...
class CommandService:
    ...
    def load_help_list(self):
        ...
        for help_data in help_json:
            self.command_registry.register_help(help_data)
    ...
    def _assign_help_to_commands(self):
        for command in self.command_registry.registry.values():
            if not command.get('help'):
                help_entry = self.command_registry.get_help_by_keyword(command.get('name', ''))
                if help_entry:
                    command['help'] = help_entry
        summary_entry = self.command_registry.get_help_by_keyword('summary')
        if summary_entry:
            self.command_registry.registry['summary'] = { 'name': 'summary', 'help': summary_entry }
