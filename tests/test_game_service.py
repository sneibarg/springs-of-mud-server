...
class TestGameService(unittest.TestCase):
    ...
    async def test_start_method(self, mock_get):
        ...
        service.game_loop.assert_called_once()
    ...
    @patch('game.GameService.requests.get')
    @patch('game.GameService.gettimeofday')
    async def test_game_loop_updates_last_time(self, mock_gettimeofday, mock_get):
    ...
    def _create_game_data_dict(self):
    ...
    def test_run_async_tests(self):
        """Run all async tests"""
        asyncio.run(self.test_game_loop_iteration())
        self.setUp()
        asyncio.run(self.test_game_loop_multiple_iterations())
        self.setUp()
        asyncio.run(self.test_start_method())
        self.setUp()
        asyncio.run(self.test_game_loop_updates_last_time())
